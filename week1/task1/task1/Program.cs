...
namespace task1
{
    class Program
    {
        static public bool Prime(int a)//создаём функцию, которая принимает одно целое число и возвращает true, если число простое и false, если число составное
        {
            if (a == 1)//единица непростое число
            {
                return false;
            }
            for(int i = 2; i < a; i++)//пробегаемся по числам от двух до а
            {
